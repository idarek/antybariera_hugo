...
function customSearchResults() {
  ...
  if (results.length >= 1) {
    ...
    for (var i = 0; i < results.length; i++) {
      ...
      var newlastmod = new Date(lastmod*1000);
      var lastmodstr = newlastmod.toLocaleDateString();
      ...
      if (templ) {
        ...
        if (templ.includes("[LASTMOD]")) {
          if (isNaN(lastmod)) {
            templ = templ.replace("[LASTMOD]", 'n/a');
          } else if (lastmod != '') {
            templ = templ.replace("[LASTMOD]", lastmodstr);
          } else {
            templ = templ.replace("[LASTMOD]", '');
          }
        }
        ...
        if (templ.includes("[SECTION]")) {
          if (section != '') {
            templ = templ.replace("[SECTION]", '['+section+'] ').format('MMMM Do YYYY, h:mm:ss a');
          } else {
